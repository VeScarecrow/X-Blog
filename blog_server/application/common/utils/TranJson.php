...
class TranJson
{
    protected static function jsonToArray($json)
    {
        if (!is_string($json) || is_null(json_decode($json, true)))
            throw new \JsonException('param is not a json string');
    ...
    {
        if (is_object($stds))
            throw new \JsonException('params not object');
    ...
    protected static function arrayRToArray($params)
    {
        $tmp = array();
        if (!is_array($params))
            throw new \JsonException('params not array');
