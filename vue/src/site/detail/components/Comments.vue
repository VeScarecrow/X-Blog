    ...
    <div class="comment-container">
        <div id="comments" ref="comments" class="clearfix">
            ...
            <div id="comment-form" class="comment-form">
                <input maxlength="12" v-model="comments.author" class="form-control input-control clearfix"
                       placeholder="姓名 (*)" value="" required/>
                <input type="email" v-model="comments.email" class="form-control input-control clearfix"
                       placeholder="邮箱 (*)" value="" required/>
                <input type="url" v-model="comments.url" class="form-control input-control clearfix"
                       placeholder="头像 (http://)" value=""/>
            ...
            <ol class="comment-list" v-if="total > 0">
                <li v-for="comment in list" :id="'li-comment-' + comment.parent.id"
                    class="comment-body comment-parent comment-odd">
                    <div :id="'comment-' + comment.parent.id">
                        <div class="comment-view">
                            <div class="comment-header">
                                <img class="avatar" :src="comment.parent.url" width="80" height="80">
                            ...
                            <div class="comment-content">
                                ...
                                <p>
                                <p v-text="comment.parent.content"/>
                            </div>
                            <div class="comment-meta">
                                <time class="comment-time" v-text="comment.parent.time"/>
                    ...
                    <div class="comment-children" v-if="comment.childrenList !== '[]'">
                        <ol class="comment-list">
                            <li v-for="item in comment.childrenList" :id="'li-comment-' + item.id"
                                class="comment-body comment-child comment-level-odd comment-odd">
                                <div :id="'comment-' + item.id">
                                    <div class="comment-view">
                                        <div class="comment-header">
                                            <img class="avatar" :src="item.url" width="80" height="80">
                                        ...
                                        <div class="comment-content">
                                        <span class="comment-author-at">
                                            <a :href="'#comment-' + (item.c_id === 0 ? item.p_id : item.c_id)"
                                               v-text="item.author_id"/>
                                        </span>
                                            <p v-text="item.content"/>
                                        </div>
                                        <div class="comment-meta">
                                            <time class="comment-time" v-text="item.time"/>
                                            <span class="comment-reply">
            ...
            <div class="lists-navigator clearfix">
                <ol class="page-navigator" v-if="total > 0">
                    <li class="prev" v-if="listQuery.pageCode !== '1'">
                        <router-link
                            :to="'?cp=' + ((listQuery.pageCode < 1) ? listQuery.pageCode : (listQuery.pageCode-1)) + '#comments'">
                            ←
                        </router-link>
                    </li>
                    <li v-for="i in total">
                    ...
                    <li class="next" v-if="listQuery.pageCode < total">
                        <router-link
                            :to="'?cp=' + ((listQuery.pageCode < total) ? (listQuery.pageCode+1) : total) + '#comments'">
                            →
                        </router-link>
                    </li>
                </ol>
    ...
    export default {
        ...
        methods: {
            ...
            findCommentsList() {
                ...
                findCommentsList(this.listQuery.pageCode, this.listQuery.pageSize, this.id, 0).then(response => {
            ...
            subComment() {
                this.comments.article_title = this.article.title;
                this.comments.article_id = this.article.id;
                if (this.comments.author.length === 0) {
                    this.$message({
                        showClose: true,
                        message: '请完善留言内容',
                        type: 'warning'
